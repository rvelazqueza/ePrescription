...
export function AIPrescriptionAssistant({
  ...
  // Usar open controlado si se provee, sino usar estado interno
  ...
  const handleSelectDiagnosis = (suggestion: DiagnosticSuggestion) => {
    ...
    if (medications.length > 0) {
      ...
      toast.info('Diagnóstico seleccionado', {
        description: 'No hay template de prescripción. Agregue medicamentos manualmente.'
      });
    }
  };

  const handleNewSearch = () => {
    // Resetear a la búsqueda inicial sin cerrar el diálogo
    ...
    setManualSearch('');
    setSearchResults([]);
    toast.info('Iniciando nueva búsqueda');
  };
  ...
  const handleApply = () => {
    ...
    const message = generatedMedications.length > 0 
      ? `Diagnóstico y ${generatedMedications.length} medicamentos agregados`
      : 'Diagnóstico agregado. Agregue medicamentos manualmente.';
  ...
  const handleSelectManual = (code: CIE10Code) => {
    ...
    if (medications.length > 0) {
      toast.success('Diagnóstico seleccionado manualmente', {
        description: `${medications.length} medicamentos sugeridos`
      });
    } else {
      toast.info('Diagnóstico seleccionado manualmente', {
        description: 'No hay template de prescripción disponible'
      });
    }
  };
                    ...
                    {generatedMedications.length > 0 ? (
                      ...
                      <Alert className="bg-blue-50 border-blue-300">
                        ...
                        <AlertDescription className="text-blue-900">
                          <strong>No hay template de prescripción para este diagnóstico.</strong>
                          <br />
