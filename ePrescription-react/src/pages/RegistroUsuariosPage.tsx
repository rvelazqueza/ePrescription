...
export function RegistroUsuariosPage({ onNavigate }: RegistroUsuariosPageProps = {}) {
  ...
  const getTotalSteps = () => {
    const perfilSeleccionado = PERFILES_USUARIO.find(p => p.value === formData.perfilUsuario);
    // Si no hay perfil seleccionado, mostrar 4 pasos (flujo completo por defecto)
    // Solo mostrar 3 pasos si se seleccionó un perfil que NO requiere colegio
    if (!perfilSeleccionado) return 4;
    return perfilSeleccionado.requiereColegio ? 4 : 3;
  };

  // Obtener el paso visual correcto para el stepper
  const getPasoVisual = (pasoActual: number, requiereColegio: boolean) => {
    if (!requiereColegio) {
      // Flujo corto: 1 → 3 → 4 (visualmente se muestra como 1 → 2 → 3)
      if (pasoActual === 1) return 1;
      if (pasoActual === 3) return 2;
      if (pasoActual === 4) return 3;
    }
    // Flujo completo: 1 → 2 → 3 → 4
    return pasoActual;
  };
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-between max-w-3xl mx-auto">
            {Array.from({ length: getTotalSteps() }, (_, i) => i + 1).map((pasoVisual, index) => {
              // Determinar la etiqueta correcta según si requiere o no colegio profesional
              const perfilSeleccionado = PERFILES_USUARIO.find(p => p.value === formData.perfilUsuario);
              // Si no hay perfil seleccionado O requiere colegio, usar flujo completo
              const requiereColegio = !perfilSeleccionado || perfilSeleccionado.requiereColegio;
              
              // Convertir paso visual a paso real para comparación
              let pasoReal: number;
              if (!requiereColegio) {
                // Flujo corto: visual 1→2→3 mapea a real 1→3→4
                if (pasoVisual === 1) pasoReal = 1;
                else if (pasoVisual === 2) pasoReal = 3;
                else pasoReal = 4; // pasoVisual === 3
              } else {
                // Flujo completo: visual 1→2→3→4 mapea a real 1→2→3→4
                pasoReal = pasoVisual;
              }
              ...
              if (requiereColegio) {
                ...
                if (pasoVisual === 1) etiqueta = "Perfil";
                else if (pasoVisual === 2) etiqueta = "Validación";
                else if (pasoVisual === 3) etiqueta = "Contacto";
                else if (pasoVisual === 4) etiqueta = "Confirmación";
              } else {
                // Flujo corto: Perfil → Contacto → Confirmación (sin validación)
                if (pasoVisual === 1) etiqueta = "Perfil";
                else if (pasoVisual === 2) etiqueta = "Contacto";
                else if (pasoVisual === 3) etiqueta = "Confirmación";
              }
              
              return (
                <div key={pasoVisual} className="flex items-center flex-1">
                  <div className="flex flex-col items-center flex-1">
                    ...
                    >
                      {pasoReal < pasoActual ? <CheckCircle2 className="w-6 h-6" /> : <span>{pasoVisual}</span>}
                    </div>
                    <p className={`text-sm mt-2 text-center font-medium ${pasoReal === pasoActual ? "text-foreground" : "text-muted-foreground"}`}>
                      {etiqueta}
                    </p>
                  </div>
                  {index < getTotalSteps() - 1 && (
                    <div className={`h-1 flex-1 mx-3 rounded transition-all ${
                      pasoReal < pasoActual ? "bg-success" : "bg-border"
                    }`} />
                  )}
                </div>
              );
