...
export function CentrosMedicosPage() {
  ...
  // Estado para asignaciones (convertido a estado mutable)
  const [asignaciones, setAsignaciones] = useState<AsignacionCentro[]>(asignacionesData);
  ...
  const handleSolicitarAsignacion = () => {
    ...
    // Buscar el centro médico seleccionado
    const centroSeleccionado = centrosMedicosData.find(c => c.id === formData.centroMedicoId);
    
    if (!centroSeleccionado) {
      toast.error("Centro médico no encontrado");
      return;
    }

    // Generar ID único para la nueva asignación
    const newId = `ASG-${String(asignaciones.length + 1).padStart(3, '0')}`;
    ...
    // Cambiar a la pestaña de asignaciones para ver la nueva solicitud
    setActiveTab("mis-asignaciones");
  };
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {asignaciones.filter(a => a.estado === "aprobada").length}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {asignaciones.find(a => a.esPrincipal)?.centroMedico.nombre.substring(0, 20) || "N/A"}
            </p>
          </CardContent>
        </Card>
