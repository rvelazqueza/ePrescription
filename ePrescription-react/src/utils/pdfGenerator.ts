...
function getCategoryLabel(category: string): string {
  const labels: Record<string, string> = {
    'narcotics': 'Estupefaciente',
    'psychotropics': 'Psicotrópico',
    'antimicrobials': 'Antimicrobiano',
...
// Función helper para obtener el HTML del badge de categoría
function getCategoryBadge(category?: string): string {
  if (!category) return '';
  const label = getCategoryLabel(category);
  return `<span class="medicine-category-badge category-${category}">${label}</span>`;
}
...
<body>
  <style>
    ...
    .category-narcotics { background: #fee2e2; color: #991b1b; border: 1px solid #dc2626; }
    .category-psychotropics { background: #fef3c7; color: #92400e; border: 1px solid #f59e0b; }
    .category-antimicrobials { background: #dbeafe; color: #1e40af; border: 1px solid #3b82f6; }
    .category-controlled { background: #fce7f3; color: #831843; border: 1px solid #db2777; }
    .category-free { background: #d1fae5; color: #065f46; border: 1px solid #10b981; }
  </style>
  ...
  <div class="prescription-container">
    ...
    <div class="header">
      ...
      <div class="header-right">
        ...
        ${prescription.bookletNumber && prescription.slipNumber ? `
        <div style="margin-top: 8px; padding: 8px; background: linear-gradient(135deg, #4f46e5, #6366f1); border-radius: 6px; border: 2px solid #312e81;">
          <div style="font-size: 8pt; color: #e0e7ff; text-transform: uppercase; letter-spacing: 1px;">Talonario</div>
          <div style="font-family: 'Courier New', monospace; color: white; font-weight: bold; font-size: 11pt;">${prescription.bookletNumber}</div>
          <div style="font-size: 8pt; color: #e0e7ff; text-transform: uppercase; letter-spacing: 1px; margin-top: 4px;">Boleta</div>
          <div style="font-family: 'Courier New', monospace; color: white; font-weight: bold; font-size: 11pt;">${prescription.slipNumber}</div>
        </div>
        ` : ''}
      </div>
    </div>
        ...
        <tr>
          ...
          <td>
            <div class="medicine-name">
              ${med.genericName}
              ${med.category ? getCategoryBadge(med.category) : ''}
            </div>
    ...
    ${prescription.bookletNumber && prescription.slipNumber ? `
    <!-- Booklet/Slip Information -->
    <div class="section-title" style="margin-top: 20px;">📋 CONTROL DE TALONARIO Y BOLETA</div>
    <div style="
    ...
    ">
      <div class="info-grid">
        <div class="info-item">
          <span class="info-label">Número de Talonario:</span>
          <span class="info-value" style="font-family: 'Courier New', monospace; color: #4f46e5; font-weight: bold;">${prescription.bookletNumber}</span>
        </div>
        <div class="info-item">
          <span class="info-label">Número de Boleta:</span>
          <span class="info-value" style="font-family: 'Courier New', monospace; color: #4f46e5; font-weight: bold;">${prescription.slipNumber}</span>
        </div>
        <div class="info-item" style="grid-column: 1 / -1;">
          <span class="info-label">Código Completo:</span>
          <span class="info-value" style="font-family: 'Courier New', monospace; color: #6366f1; font-weight: bold; font-size: 12pt;">${prescription.fullSlipNumber || `${prescription.bookletNumber}-${prescription.slipNumber}`}</span>
        </div>
      </div>
      <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid #c7d2fe; font-size: 9pt; color: #64748b;">
        ℹ️ Receta controlada: Este talonario y número de boleta son únicos y rastreables para garantizar la trazabilidad y seguridad de la prescripción.
      </div>
    </div>
    ` : ''}
...
/**
 * Generar PDF con múltiples recetas con saltos de página
 * Emula boletas físicas con una receta por página
 */
export function generateMultiplePrescriptionsPDF(prescriptionsData: EmittedPrescriptionData[]): void {
  if (prescriptionsData.length === 0) {
    alert('No hay recetas para imprimir');
    return;
  }
  ...
  const prescriptionsHTML = prescriptionsData.map((prescriptionData, index) => {
    const { prescription, medicines } = prescriptionData;
    const isLast = index === prescriptionsData.length - 1;
    
    return `
    <div class="prescription-page" ${!isLast ? 'style="page-break-after: always;"' : ''}>
      <!-- Header -->
      <div class="header">
        <div class="header-left">
          <div class="hospital-name">${prescription.medicalCenter}</div>
        ...
        <div class="header-right">
          ...
          <div class="prescription-date">${prescription.issueDate} - ${prescription.issueTime}</div>
          <div class="prescription-date">Estado: ${getStatusLabel(prescription.status)}</div>
          <div class="prescription-badge">Receta ${index + 1} de ${prescriptionsData.length}</div>
          ${prescription.bookletNumber && prescription.slipNumber ? `
          <div style="margin-top: 8px; padding: 8px; background: linear-gradient(135deg, #4f46e5, #6366f1); border-radius: 6px; border: 2px solid #312e81;">
            <div style="font-size: 8pt; color: #e0e7ff; text-transform: uppercase; letter-spacing: 1px;">Talonario</div>
            <div style="font-family: 'Courier New', monospace; color: white; font-weight: bold; font-size: 11pt;">${prescription.bookletNumber}</div>
  ...
  // Generar HTML del documento completo
  const htmlContent = `
<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Recetas Médicas - ${prescriptionsData.length} receta${prescriptionsData.length > 1 ? 's' : ''}</title>
  <style>
    * {
      margin: 0;
      padding: 0;
...
<body>
  ...
  <button class="print-button no-print" onclick="window.print()">🖨️ Imprimir / Guardar PDF</button>
  <div class="page-counter no-print">📄 ${prescriptionsData.length} receta${prescriptionsData.length > 1 ? 's' : ''} lista${prescriptionsData.length > 1 ? 's' : ''} para imprimir</div>
  ...
  <script>
    // Auto-abrir diálogo de impresión después de cargar
    window.onload = function() {
      console.log('PDFs de recetas médicas cargados:', ${prescriptionsData.length}, 'recetas');
    };
...
/**
 * Descargar múltiples recetas como un solo PDF
 * Abre diálogo de impresión automáticamente para guardar como PDF
 */
export function downloadMultiplePrescriptionsPDF(prescriptionsData: EmittedPrescriptionData[]): void {
  if (prescriptionsData.length === 0) {
    alert('No hay recetas para descargar');
    return;
  }
  ...
  // Generar HTML con todas las recetas (mismo código que generateMultiplePrescriptionsPDF)
  const prescriptionsHTML = prescriptionsData.map((prescriptionData, index) => {
    const { prescription, medicines } = prescriptionData;
    const isLast = index === prescriptionsData.length - 1;
  ...
  const patientName = prescriptionsData[0]?.prescription?.patientName || 'Paciente';
  const sanitizedName = patientName.replace(/[^a-zA-Z0-9]/g, '_');
  const fileName = `Recetas_${sanitizedName}_${prescriptionsData.length}_recetas`;

  // Generar HTML completo (mismo estilo que generateMultiplePrescriptionsPDF pero con auto-print)
  const htmlContent = `
<!DOCTYPE html>
<html lang="es">
<head>
